...
import { speakText, stopSpeaking, createSpeechRecognizer, isSpeechRecognitionSupported, unlockAudio } from '../services/speech';
...
export function SpeakConversation() {
        ...
        if (data.messages.length === 1 && data.messages[0].role === 'assistant' && !data.endedAt) {
          ...
          setTimeout(() => {
            ...
            speakText({
              ...
              // Safari 可能阻止自动播放，这是正常的，用户可以手动点击播放
              console.log('[Speak] 开场白自动播放失败:', error.message);
            }).finally(() => {
  ...
  const handleRecordingStart = () => {
    if (conversationState === 'speaking' || conversationState === 'thinking' || isPlayingAudio) return;

    // 停止当前播放的音频
    stopSpeaking();
    setIsPlayingAudio(false);

    // 解锁音频播放（Safari 需要，必须在同步上下文中调用）
    unlockAudio();
  ...
  const sendFreeTalkMessage = async (text: string) => {
    ...
    try {
      ...
      speakText({
        ...
        console.error('[FreeTalk] 播放失败:', error.message);
      }).finally(() => {
        setIsPlayingAudio(false);
        // AI 回复完成后，继续监听
        if (isFreeTalkModeRef.current) {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      speakText({
        ...
        console.error('[Speak] 播放失败:', error.message);
        // 播放失败时提示用户可以手动点击播放
        if (error.message?.includes('被浏览器阻止')) {
          toast.info('音频被阻止，请点击消息旁的播放按钮');
        }
      }).finally(() => {
        setIsPlayingAudio(false);
      });
  ...
  const playMessage = async (message: SpeakMessage) => {
    ...
    try {
      ...
      console.error('[Speak] 播放失败:', error);
  ...
  return (
    <div className="container mx-auto px-4 py-4 max-w-2xl h-screen flex flex-col">
      ...
      <div className="border-t pt-4">
        {/* 文本输入 - 自由对话模式下隐藏 */}
        {!isFreeTalkMode && !isConversationEnded && (
          <div className="flex gap-2 mb-4">
            <input
              type="text"
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              onFocus={() => unlockAudio()} // 聚焦时解锁音频（Safari 需要）
            ...
            <button
              className={`w-20 h-20 rounded-full flex flex-col items-center justify-center transition-all ${
                recordingState === 'recording'
                  ...
                  : conversationState !== 'idle' || endingConversation || isPlayingAudio || isConversationEnded
              ...
              disabled={conversationState !== 'idle' || endingConversation || isPlayingAudio || isConversationEnded}
            >
              {recordingState === 'recording' ? (
                <>
          ...
          <button
            className={`w-20 h-20 rounded-full flex flex-col items-center justify-center transition-all ${
              isFreeTalkMode
                ...
                : conversationState !== 'idle' || endingConversation || !speechSupported || isPlayingAudio || isConversationEnded
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                : 'bg-gray-600 text-white hover:bg-gray-700'
            }`}
            onClick={isFreeTalkMode ? stopFreeTalkMode : startFreeTalkMode}
            disabled={conversationState !== 'idle' || endingConversation || !speechSupported || isPlayingAudio || isConversationEnded}
          >
            {isFreeTalkMode ? (
              <>
        ...
        <div className="text-center mt-4">
          ...
          {!speechSupported && !isConversationEnded && (
